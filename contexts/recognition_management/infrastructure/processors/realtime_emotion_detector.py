# Ubicación: contexts/recognition_management/infrastructure/processors/realtime_emotion_detector.py
import cv2
import numpy as np
...
# Servicios del avatar (callbacks por defecto si no nos pasan otros)
from contexts.avatar_management.domain.services.avatar_animation_service import AvatarAnimationService
from contexts.avatar_management.domain.services.landmark_smoothing_service import LandmarkSmoothingService
...
_smoother = LandmarkSmoothingService(alpha=0.6)
...
def detect_real_time_emotions_and_landmarks(
    *,
    ...
    - Si no pasas callbacks, usa AvatarAnimationService por defecto.
    - NO muestra la cámara a menos que show_camera_window=True.
    """

    global _current_emotion, _stop_flag

    # Callbacks por defecto: pintar el avatar
    ...
    def _default_on_landmarks(lms: np.ndarray) -> None:
        try:
            AvatarAnimationService.update_landmarks(lms)
        except Exception as e:
            print("[ERROR] update_landmarks falló:", e, flush=True)
    ...
    try:
        # Muestra una base neutral al inicio para asegurar ventana creada
        try:
            AvatarAnimationService.generate_animation(_current_emotion)
        except Exception:
            pass
        ...
        while not _stop_flag:
            ...
            if frame_id % max(1, fer_every_n_frames) == 0:
                ...
                # solo disparamos si cambia, para evitar “rebotes”
            ...
            if results_any and getattr(results_any, "multi_face_landmarks", None):
                ...
                lms = _smoother.smooth(lms)

                # Asegurar (N,3); si llega (T,N,3) -> último frame
                lms = np.asarray(lms)
                if lms.ndim == 3:
                    lms = lms[-1]
                if lms.ndim != 2 or lms.shape[0] < 3:
                    # frame no confiable
                    pass
                else:
                    on_landmarks(lms)
...
def run_realtime_detector(
    *,
    ...
    """Alias con misma firma que el import de app.py."""
